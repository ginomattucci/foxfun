class BestGuessAttemptController < ApplicationController
  ...
  def create
    ...
    if attempt.save
      ...
      if best_guess.tournament?
        ...
        return redirect_to best_guess_path(best_guess), flash: { notice: 'Inscrito com sucesso! Clique em continuar para dar seu palpite' }
      end
    else
  ...
  def update
    ...
    if policy(@attempt).update?
      ...
      return redirect_to best_guess_path(@best_guess), flash: { alert: 'Não autorizado' }
    end
